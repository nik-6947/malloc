...
static trace_t *read_trace(char *tracedir, char *filename)
{
    ...
    if(fscanf(tracefile, "%u", &(trace->sugg_heapsize))) /* not used */
    if(fscanf(tracefile, "%u", &(trace->num_ids)))     
    if(fscanf(tracefile, "%u", &(trace->num_ops)))     
    if(fscanf(tracefile, "%u", &(trace->weight)))        /* not used */
